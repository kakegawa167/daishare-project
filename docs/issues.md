...
| 進捗         | 2026-07-02: Realtime INSERT が実機で配信されていることを確認（自分の送信メッセージが Realtime でも届き、楽観更新と二重表示になる ERR-030 が発生 → 裏を返せば INSERT イベントは受信できている）。ERR-030 修正で自分発はスキップ。**残: 2アカウント間（他ユーザーからの受信）のリアルタイム反映テスト** |
| 解決日時     | —                                                                                             |
| 解決方法     | —                                                                                             |
...
| 解決日時     | 2026-07-01                                                                    |
| 解決方法     | `xcodebuild` + `-allowProvisioningUpdates` で Development Build をビルドし、`xcrun devicectl` で iPhone に直接インストール。`npx expo start` で Metro を起動すれば JS 変更は Fast Refresh で自動反映される（ERR-024 参照）。アバターアップロードも base64 方式に修正済み（ERR-017） |
