...
| 発生日時     | 2026-06-24                                                                                                            |
| ステータス   | `open`                                                                                                                |
| 課題内容     | 設計では「借主はキャンセル不可・貸主がキャンセルする」だが、`POST /rental-requests/{id}/cancel` の実装（`rental_requests.py` L229）は `renter_id == user_id` の借主のみキャンセル可になっている。モバイルアプリ側では借主のキャンセルボタンを削除済みだが、APIとして借主がキャンセルできてしまう状態 |
| 影響範囲     | リクエストのキャンセルフロー。UIでは発生しないが直接APIを叩けば借主がキャンセル可能                                   |
| 必要なアクション | `backend/app/routers/rental_requests.py` の `cancel_request` エンドポイントを修正:<br>`if str(r.renter_id) != user_id:` → `if str(r.cart.owner_id) != user_id:` に変更し、貸主のみキャンセルできるようにする |
| 解決日時     | —                                                                                                                     |
| 解決方法     | —                                                                                                                     |
